...
async function applyAutoDetect(config, ctx) {
  ...
  for (const candidate of plan.sources) {
    ...
    for (const route of candidate.providers) {
      ...
      // 实测窗口不需要凭据：它只统计经过本实例的调用。
      if (candidate.window !== undefined) {
        usable.push({ route, ref: null })
        continue
      }
      let hit
      for (const ref of refs) {
        const secret = await resolveRef(ref)
        if (secret !== undefined) {
          hit = { ref, from: secret.source ?? 'resolved' }
          break
        }
      }
      if (hit !== undefined) usable.push({ route, ...hit })
      else skipped.push({ id: candidate.id, route, reason: 'no-credential', tried: refs })
    }
    if (usable.length === 0) continue
    const override = { id: candidate.id, providers: usable.map(entry => entry.route) }
    if (candidate.region !== undefined) override.region = candidate.region
    // 路由点名的凭据引用与预设不同（例如 MOONSHOT_CN_API_KEY）时，用它，不要求用户再配一遍。
    const routeRef = usable.find(entry => entry.ref !== null && preset !== undefined && entry.ref !== preset.bearerRef)
    if (routeRef !== undefined && preset !== undefined && typeof preset.cookieRef !== 'string') override.bearerRef = routeRef.ref
    const [source] = normalizeSources([override], ctx, config)
    if (source === undefined) continue
    source.detected = { ...candidate.detected, credentialRef: routeRef?.ref ?? null }
