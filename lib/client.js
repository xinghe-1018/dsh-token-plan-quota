 ...
 * 面板：一律 portal 到 body 用 fixed 定位（宿主工具栏的 transform/backdrop-filter 会
 * 劫持 fixed 包含块，body 下才是视口坐标系）。按住标题栏可拖到任意位置悬浮（移动只走
 * transform，松手才提交坐标；位置记在 localStorage，跨刷新记住），双击标题栏归位贴回徽标。
...
`;
    ...
    function Badge({ copy, watchActive }) {
      ...
      /** 拖起瞬间冻结的视口坐标：拖拽期间面板用 left/top=冻结值 + transform 偏移，
       *  轮询重渲染不会把样式打回锚定位。 */
      ...
      const beginDrag = (event) => {
        ...
        setDragXY({ x: Math.round(rect.left), y: Math.round(rect.top) });
      ...
      React.useEffect(() => {
        ...
        const onDown = (event) => {
          ...
          // 面板已 portal 到 body：点它内部不算「点外面」。
          const node = panelRef.current;
          const inPanel = node !== null && typeof node.contains === "function" && node.contains(target);
          const inRoot = root.current !== null && root.current.contains(target);
      ...
      if (open && children.length > 0) {
        ...
        let panelStyle;
        if (dragXY !== null) {
          panelStyle = { position: "fixed", left: `${dragXY.x}px`, top: `${dragXY.y}px`, bottom: "auto" };
        } else if (floatPos !== null) {
          panelStyle = { position: "fixed", left: `${floatPos.x}px`, top: `${floatPos.y}px`, bottom: "auto" };
        } else if (vw !== null && vh !== null && root.current !== null
        ...
        const panelEl = React.createElement("div", {
          ...
          "data-float": floatPos !== null || dragXY !== null ? "1" : undefined,
          style: panelStyle,
        },
        ...
        children.push(typeof React.createPortal === "function" && typeof document !== "undefined" && document.body !== undefined
          ? React.createPortal(panelEl, document.body)
          : panelEl);
      }
      return React.createElement("span", { ref: root, className: "tpq" }, children);
    }
