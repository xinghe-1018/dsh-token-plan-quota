 ...
 *
 * 观感：徽标做成工具行里的幽灵按钮（无边框、灰字、彩色小圆点表状态），
 * 不再用彩色描边抢视线。当前模型没有可显示的额度源时徽标整个隐藏。
 *
...
.tpq-chip{display:inline-flex;align-items:center;gap:6px;height:24px;padding:0 7px;border-radius:7px;
...
.tpq-dot{width:7px;height:7px;border-radius:50%;flex:none;
 background:linear-gradient(135deg,#2fa96b,#4176e6)}
.tpq-chip[data-sev=muted] .tpq-dot,.tpq-chip[data-sev=bad] .tpq-dot{background:var(--dsw-alias-label-tertiary,#8a8a8a)}
.tpq-chip[data-sev=warn] .tpq-dot{background:var(--dsw-alias-state-warn-primary,#c98a12)}
.tpq-chip[data-sev=danger] .tpq-dot{background:var(--dsw-alias-state-error-primary,#d64545)}
.tpq-chip[data-busy] .tpq-dot{animation:tpq-pulse 1s ease-in-out infinite}
@keyframes tpq-pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.3;transform:scale(.72)}}
.tpq-name{color:var(--dsw-alias-label-secondary,inherit)}
.tpq-num{font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary,inherit)}
.tpq-pill{font-size:10px;line-height:15px;padding:0 6px;border-radius:999px;flex:none;
 border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.3));color:var(--dsw-alias-label-tertiary,inherit)}
.tpq-pill[data-kind=official]{border:none;color:#fff;background:linear-gradient(90deg,#2f9bdc,#4176e6)}
...
.tpq-speed{font-size:11px;font-variant-numeric:tabular-nums;min-width:5.6em;text-align:right;flex:none;
 color:var(--dsw-alias-brand-primary,#4176e6)}
.tpq-tag{font-size:11px;color:var(--dsw-alias-label-tertiary,inherit);font-variant-numeric:tabular-nums}
.tpq-panel{position:absolute;bottom:calc(100% + 6px);left:0;z-index:60;width:min(380px,88vw);
 ...
 box-shadow:0 12px 32px rgba(0,0,0,.26),0 2px 6px rgba(0,0,0,.12)}
.tpq-title{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px;
 padding-bottom:8px;border-bottom:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.16))}
.tpq-title>span:first-child{font-weight:600;font-size:13px}
.tpq-card{padding:8px 10px;margin-bottom:8px;border-radius:10px;
 background:var(--dsw-alias-bg-layer-1,rgba(128,128,128,.07));
 border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.14))}
.tpq-card:last-child{margin-bottom:0}
.tpq-h{display:flex;align-items:center;gap:6px;font-weight:600;margin-bottom:2px}
.tpq-big{font-size:14px;font-weight:600;font-variant-numeric:tabular-nums;
...
.tpq-bar-lg>i{position:absolute;inset:0 auto 0 0;border-radius:999px;background-repeat:no-repeat;
 transition:width .45s ease,background .6s ease}
.tpq-row{display:flex;justify-content:space-between;gap:12px;padding:1px 0}
.tpq-row>span:first-child{color:var(--dsw-alias-label-tertiary,inherit)}
.tpq-row>span:last-child{font-variant-numeric:tabular-nums}
...
.tpq-item>span:last-child{font-variant-numeric:tabular-nums}
...
.tpq-updated{display:block;width:100%;margin-top:6px;padding:2px 0;border:none;background:transparent;
...
.tpq-meta{font-size:11px;color:var(--dsw-alias-label-tertiary,inherit);word-break:break-all}
...
`;
    ...
    function Chip({ card, copy, busy, expanded, onToggle, modelText, speedTps }) {
      ...
      // 进度条表达「余量」：绿→蓝渐变，宽度=剩余百分比；用量变化由 CSS 过渡平滑。
      ...
      return React.createElement("button", {
        ...
        React.createElement("span", { className: "tpq-dot" }),
    ...
    function CardDetail({ card, copy }) {
      ...
      if (isMoney(card)) {
        ...
        if (typeof card.tokens === "number") {
          ...
          bits.push(`${scope} ${formatTokens(card.tokens)} · ${card.calls ?? 0} ${copy.calls}`);
          if (card.remainingDays !== undefined) bits.push(`· ${copy.daysLeft}${card.remainingDays}${copy.daysUnit}后重置`);
        }
      } else {
        if (typeof card.remaining === "number") bits.push(`${copy.remaining} ${card.unit === "Credits" ? formatCredits(card.remaining) : formatTokens(card.remaining)}${card.unit ? ` ${card.unit}` : ""}`);
      ...
      return React.createElement("div", { className: "tpq-card" },
        React.createElement("div", { className: "tpq-h", title: headerTitle },
          ...
          card.action ? React.createElement("span", { className: "tpq-meta" }, card.action) : null),
        bits.length > 0
          ? React.createElement("div", { className: "tpq-row" },
              React.createElement("span", { className: "tpq-big" }, bits[0]),
              bits.length > 1
                ? React.createElement("span", { className: "tpq-meta", style: { alignSelf: "center", textAlign: "right" } }, bits.slice(1).join(" "))
                : null)
          : null,
    ...
    function Badge({ copy, watchActive }) {
      ...
      if (open && children.length > 0) {
        children.push(React.createElement("div", { className: "tpq-panel", role: "dialog", key: "panel" },
          React.createElement("div", { className: "tpq-title" },
            React.createElement("span", null, copy.detailTitle),
            modelText === null ? null : React.createElement("span", { className: "tpq-meta" }, `${copy.modelNow}: ${modelText}`)),
          throughput !== null && throughput !== undefined
            ? React.createElement(ThroughputBlock, { tp: throughput, copy })
            : null,
          panelCards.map(card => React.createElement(CardDetail, { key: card.id, card, copy })),
