...
export const PLATFORM_RULES = [
  {
    ...
    // 控制台网关给官方余量；实测窗口只作**兜底**（Cookie 掉了才出现）。
    // 两张一起挂就是"同一个供应商两张余量卡"——一家只该有一张额度卡。
    presets: ['token-plan-console', 'token-plan-window'],
    fallbackPresets: ['token-plan-window'],
...
function matchRule(route, profile, rule) {
  ...
  const matchedHost = hostMatches(rule, host)
  if (matchedHost !== undefined) return { by: 'baseURL', host: matchedHost }
  // **知道 host 就只信 host**。名字像不是一回事：`deepseek-modlens` 这类路由的 baseURL 指向
  // 本地代理，只是路由名带了别家牌子；按名字认就会把别家的余额数字顶在它头上。
  // 只有 host 完全拿不到时，才降级用路由名 / Key 前缀这两条弱线索。
 ...
 *   - `disabledPresets`：用户用 `enabled:false` 明确关掉的**源名**（不是路由）：这些源一概不再自动开，
 *     但同一路由的实测兜底窗口不受牵连——关一条源不等于关这个供应商；
 *   - `rules`：覆盖默认规则表（测试用）。
...
export function detectSources(input) {
  ...
  const disabled = new Set((Array.isArray(input?.disabledPresets) ? input.disabledPresets : []).map(String))
  ...
  for (const route of routes) {
    ...
    const region = hit.matched.host !== undefined && hit.rule.regionByHost !== undefined
      ...
      : undefined
    const fallbacks = new Set(hit.rule.fallbackPresets ?? [])
    for (const preset of hit.rule.presets ?? []) {
      if (disabled.has(preset)) {
        skipped.push({ preset, route: id, reason: 'disabled-by-config', by: hit.matched.by })
        continue
      }
      const bucket = grouped.get(preset) ?? { providers: [], region, by: hit.matched, ruleKey: hit.rule.key, fallback: fallbacks.has(preset) }
  ...
  for (const [preset, bucket] of grouped) {
    ...
    sources.push({
      ...
      // 兜底源（实测窗口这类）：只有同路由的官方源没开起来时才该出现。凭据在调用方才知道，
      // 所以这里只打标记，去留由 index 定——一家同时挂"官方余量 + 实测"两张卡就是重复显示。
      fallback: bucket.fallback === true,
      detected: { by: bucket.by.by, rule: bucket.ruleKey, host: bucket.by.host ?? null },
    })
  }
  for (const id of uncovered) {
    if (disabled.has(`window:${id}`)) {
      // 仍要在诊断里留痕：路由"没被认出"和"用户关了它的兜底源"是两回事，别让人去查规则表。
      skipped.push({ preset: `window:${id}`, route: id, reason: 'disabled-by-config', by: 'fallback-window' })
      continue
    }
    sources.push({
      id: `window:${id}`,
