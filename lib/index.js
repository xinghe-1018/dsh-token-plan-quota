...
async function applyAutoDetect(config, ctx) {
  ...
  // 兜底源让路：官方源已经接住的路由，实测窗口就不再出现——**一家只留一张额度卡**。
  const served = new Set()
  for (const candidate of planned) {
    if (candidate.fallback === true) continue
    for (const route of candidate.providers) served.add(route)
  }
  const keep = planned.filter(candidate => {
    if (candidate.fallback !== true) return true
    if (candidate.providers.every(route => served.has(route))) {
      skipped.push({ preset: candidate.id, route: candidate.providers.join(','), reason: 'covered-by-official', by: candidate.detected.by, tried: [] })
      return false
    }
    return true
  })
  // **同一个源名只允许一张卡**。重复开出同名源会有两个后果：面板里并排两张几乎一样的卡
  // （React key 也撞），而且它们共用 TTL 缓存条目互相覆盖——表现就是"两个余量窗口、数字还不一样"。
  ...
  for (const candidate of keep) {
    ...
    source.detected = { ...candidate.detected, credentialRef: candidate.ref }
