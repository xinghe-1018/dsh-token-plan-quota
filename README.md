...
**点输入框不会关掉明细**：落在输入卡片内（输入框、工具行、发送按钮；锚点是宿主的
`[data-composer-card]`）都不算「点外面」——边看余量边写 prompt 是常态；点对话区/侧栏或按 Esc 才关。
...
```powershell
node test/host.mjs     # 186 项：签名对照官方 SDK、官方字段抽取、窗口账本滚动与基线、吞吐速度数学、控制台网关回环（sec_token 自动获取+三接口）、多计量条数学（各窗口独立算百分比、没读数的窗口不存在、实测计量剥百分比）、panelScope、Bearer 回环链路、并发与 TTL、观测解析（注：测试固定写 C:\test-dsh-home，需在可写该路径的终端里跑）
node test/client.mjs   # 91 项：座位注册、样式与 CDN 字体 link 注入、徽标跟随模型切换、panelScope 过滤与 all 回退、速度标签新鲜度（纯文本）、徽标无圆点无表情符号、多计量条只渲染真有读数的次级窗口、点输入卡片不关明细、实测卡误喂分母也不出条不出百分比、面板 portal 到 body、拖拽/缩放手柄与提示、面板卡交错渐入序号、混排灰列走 UI 栈、手势态禁碰 animation（防重播渐入）、紧凑面板（一行摘要/一行吞吐/每供应商一行重试）、无绑定隐藏、缺服务退回全量
```
