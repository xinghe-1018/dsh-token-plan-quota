...
## Phase 3 — 验收（可证伪性实测：还原修复前 README 后 check-docs exit=1，逐条指名 10 条违规行（= 5 个去重目标 × 2 份 README））

- [x] **T005** `npm run check` 全绿；`npm pack --dry-run` 仍 **24 files**（FR-004 / SC-003）
- [x] **T006** 一个提交：`fix(docs): README 不再链接不随包发布的文件，并加发布面检查`
...
改动落在 6 个文件（两份 README、`scripts/check-docs.mjs`、以及 specs 三件套），不并行。**不新增 host/client 用例**（不涉及插件运行时行为），
但 T004 是硬要求——新增的检查必须被证明**能失败**。

## 备注

- **合并顺序**：002 叠在**未合入**的 001 之上（`main..HEAD` 同时含 001 的提交），合并时先 001 后 002。
- **提交正文里的验证声明**要贴输出、或指向记录了输出的工件（本轮初版的提交正文只作了声称，属同类瑕疵）。
...
## Phase 5 — Lens 3 的工件发现（2026-09-18）

- [x] **T011** 位置行号腐烂（plan 前提 6 的「第 292 行」、问题段的「L66」）→ 一律改章节锚点；这是 001 T013 的回归
- [x] **T012** plan §9 五步勾选；`FR-002` 措辞与重写后的实现同步（含图片与四种写法）
- [x] **T013** 校正引用与单位：T006 的提交标题少了「到」；「10 处」＝5 个去重目标 × 2 份 README；「两个文件」实为 6 个
- [x] **T014** 记录合并顺序（002 叠在未合入的 001 上）与「提交正文要带证据或指向工件」
