...
关掉重开、刷新页面都记住位置；**右下角 L 形手柄拖拽缩放**（下限 280×160，上限夹到视口边，尺寸和位置
一起记进 localStorage；手柄只在悬浮态出现，锚定态保持默认尺寸）；双击标题栏归位贴回
徽标并清掉自定义尺寸；窗口变化自动重算/夹回视口。
...
node test/client.mjs   # 74 项：座位注册、样式与 CDN 字体 link 注入、徽标跟随模型切换、panelScope 过滤与 all 回退、速度标签新鲜度（纯文本）、徽标无圆点无表情符号、面板 portal 到 body、拖拽/缩放手柄与提示、面板卡交错渐入序号、混排灰列走 UI 栈、手势态禁碰 animation（防重播渐入）、紧凑面板（一行摘要/一行吞吐/每供应商一行重试）、无绑定隐藏、缺服务退回全量
```
