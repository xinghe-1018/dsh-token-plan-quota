...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 点在输入卡片里（含输入框本身、工具行、发送按钮）都不关明细——边看余量边写 prompt
     *  是常态。`data-composer-card` 是宿主 InputBar 的稳定锚点；后面几个选择器兜住
     *  弹窗/其它外壳里的编辑区。点真正的别处（对话区、侧栏）才关。 */
    const KEEP_OPEN_SELECTOR = "[data-composer-card], textarea, input, select, [contenteditable]";
