...
（蓝渐变药丸）/「实测」（灰药丸）标签，大号数值 + 绿→蓝渐变余量条（宽度带 0.5s 弹性缓动，
不硬跳），来源与口径长句收进 tooltip；吞吐压成一行（定宽 tabular 数字，不随位数抖动）；
模型明细最多 6 行；底部只有一个低调的「更新于 hh:mm:ss」——点它强制刷新官方源。
强制刷新时徽标数值做呼吸动画（纯 opacity，宽度零抖动）；面板点开时 6px 上浮淡入，
带 1px 内高光边。面板宽度 380px、高度上限 52vh。
...
node test/client.mjs   # 66 项：座位注册、徽标跟随模型切换、panelScope 过滤与 all 回退、速度标签新鲜度（纯文本）、徽标无圆点无表情符号、紧凑面板（一行摘要/一行吞吐/每供应商一行重试）、无绑定隐藏、缺服务退回全量
```
