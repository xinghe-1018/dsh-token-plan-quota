 ...
 * transform，松手才提交坐标；位置记在 localStorage，跨刷新记住），右下角 L 形手柄拖拽
 * 缩放（280×160 起，夹到视口边，尺寸同样记住），双击标题栏归位贴回徽标。
...
.tpq-resize{display:none}
.tpq-panel[data-float=1] .tpq-resize{display:block;position:absolute;right:1px;bottom:1px;width:16px;height:16px;
 cursor:nwse-resize;touch-action:none}
.tpq-resize::after{content:"";position:absolute;right:3px;bottom:3px;width:7px;height:7px;
 border-right:1.5px solid var(--dsw-alias-label-tertiary,rgba(128,128,128,.6));
 border-bottom:1.5px solid var(--dsw-alias-label-tertiary,rgba(128,128,128,.6));
 border-bottom-right-radius:3px;opacity:.45;transition:opacity .15s ease}
.tpq-resize:hover::after{opacity:1}
.tpq-panel[data-resizing]{cursor:nwse-resize}
.tpq-panel[data-float=1] .tpq-updated{padding-right:16px}
    ...
    function Badge({ copy, watchActive }) {
      ...
      if (open && children.length > 0) {
        ...
        const panelEl = React.createElement("div", {
          ...
          // 缩放手柄挂外层（不随内容滚动），仅悬浮态可见——锚定态保持默认尺寸。
