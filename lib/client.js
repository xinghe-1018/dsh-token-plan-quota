...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function cardValueText(card, copy) {
      ...
      // 上游直接返回了比例、档位额度那一路没取到：报「已用 x%」（官方真值），
      // 不折算绝对余量、不画条 —— 缺什么就是不报什么。
      if (typeof card.usedPercent === "number") return copy.usedOnly.replace("{p}", Math.round(card.usedPercent * 10) / 10);
      return copy.noData;
    }
     ...
     *  条**只认 remainingPercent**：那个字段由宿主在有官方分母的前提下派生出来；
     *  只有 usedPercent 的卡（上游直给比例、没取到档位额度）不画条 —— 条在视觉上是
     *  "余量/额度"，没有绝对分母支撑就会和档位上限值混淆（宪法原则 I v2.0.0）。 */
