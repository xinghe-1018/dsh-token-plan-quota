...
function assertDeclaredHost(rawUrl, config, sourceId) {
  ...
  // 自己解析，不用 `hostOf`：后者对裸字符串（`not-a-url`）也会当主机返回，用它判"是不是绝对 URL"
  // 会把拼错的地址误判成"未声明的另一台主机"，报错指向完全不同的方向。
  const match = /^(https?):\/\/([^/?#]+)/.exec(text)
  if (match === null) throw new SourceError('BadUrl', `出站地址不是绝对 URL：${text.slice(0, 120)}`, sourceId)
  const [, scheme, host] = match
  const origin = `${scheme}://${host}`
  // 回环是本仓库**测试与本地探针**的明文例外（宪法 VI：测试只允许回环 HTTP）。它不是"出站主机"，
  // 声明集合也不可能覆盖它——所以先放行，再要求其余一律 https。注意 host 带端口（`127.0.0.1:9`）。
  if (/^(127\.\d+\.\d+\.\d+|localhost|\[?::1\]?)(:\d+)?$/i.test(host)) return origin
  if (scheme !== 'https') throw new SourceError('BadUrl', `出站地址必须是 https（只有回环允许 http）：${text.slice(0, 120)}`, sourceId)
...
async function postForm(url, body, timeoutMs) {
  ...
  if (response.status === 401 || response.status === 403) {
    throw new SourceError(String(response.status), '鉴权被拒（检查 Key 权限与所属账号）', url, text.slice(0, 600), [body])
  }
  return text
}
...
function parseEnvelope(text, sourceId, envelopeOk, secrets = []) {
  ...
  if (failure !== undefined) throw new SourceError(failure.code, failure.message, sourceId, configlessTrim(payload), secrets)
  return payload
}
