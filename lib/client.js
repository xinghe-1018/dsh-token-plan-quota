 ...
 * 老宿主没有 react-dom 种子词时退回内联 CSS 锚定，拖拽悬浮自动禁用。
...
`;
    ...
    function Badge({ copy, watchActive }) {
      ...
      const beginDrag = (event) => {
        ...
        if (node === null || event.button !== 0 || !canPortal || typeof node.getBoundingClientRect !== "function") return;
        ...
        // 掐掉可能仍在跑的入场动画（动画的 transform 优先级高于行内样式，会抢拖拽）。
        // 用 cancel 而不是 CSS 开关 animation-name——后者从 none 变回有值会被浏览器
        // 当作新动画重播（就是「松手后重播渐入」那个坑）。
        try {
      ...
      if (open && children.length > 0) {
        ...
        const panelEl = React.createElement("div", {
          ...
          "data-float": canPortal && (floatPos !== null || dragXY !== null) ? "1" : undefined,
          style: panelStyle,
        },
