...
function effectiveConfig(rowConfig, ctx) {
  ...
  merged.sources = normalizeSources(merged.sources, ctx)
 ...
 */
function normalizeSources(raw, ctx) {
...
/** usage + subscription + quota-config（按档位）→ 官方余量卡（剩余 = weekly × (1 − 已用比例)）。 */
function buildConsoleCard(source, usage, quota, subscription) {
  ...
  const card = {
    ...
    remaining: weeklyTotal !== undefined && weekRatio !== undefined
      ...
      : undefined,
    expiresAt: toEpochMs(usage.per1WeekResetTime),
    items: [],
    // 两个窗口各是一条计量：顶层字段仍是 7 天窗口（老前端与工具摘要读它），
    // meters 给新前端逐条渲染（5 小时窗口此前只能挤在 extra 里当文字）。
    meters: [
      ...(weeklyTotal !== undefined || weekRatio !== undefined
        ? [{
          key: 'weekly',
          label: '7 天窗口',
        ...
        : []),
      ...(fiveTotal !== undefined || fiveRatio !== undefined
        ? [{
          key: 'fiveHour',
          label: '5 小时窗口',
          ...
          unit: 'Credits',
          total: fiveTotal,
          remaining: fiveTotal !== undefined && fiveRatio !== undefined
            ? round(Math.max(0, fiveTotal * (1 - Math.min(1, fiveRatio))))
            : undefined,
    ...
    extra: {
      ...
      fiveHourResetAt: toEpochMs(usage.per5HourResetTime),
    },
  }
  if (card.remaining === undefined && card.total === undefined) {
    card.emptyReason = '网关响应里没有识别到 per1WeekPercentage/weekly 字段（开 debug 看 shape）'
  }
  return card
}
...
function buildSingleCard(source, payload) {
  const remaining = toNumber(pickFirst(payload, source.extract?.remaining ?? []))
  const total = toNumber(pickFirst(payload, source.extract?.total ?? []))
  const extra = {}
  ...
  const card = {
    ...
    unit: pickString(payload, source.extract?.unit ?? []) ?? (source.metric === 'money' ? 'CNY' : ''),
    remaining,
    total,
    used: remaining !== undefined && total !== undefined ? round(Math.max(0, total - remaining)) : undefined,
    items: [],
    extra,
  }
  if (remaining === undefined && total === undefined) card.emptyReason = '响应里没有识别到金额字段（开 debug 核对字段名）'
  return card
}
...
function finalizeCard(card) {
  ...
  if (typeof card.errorCode === 'string' && card.hint === undefined) card.hint = hintFor(card.errorCode)
  return card
}
...
async function querySource(source, config, credentials) {
  ...
  if (source.type === 'http' && typeof source.bearerRef === 'string' && source.bearerRef !== '' && source._bearer === undefined) {
    ...
    card.hint = hintFor(card.errorCode)
    return finalizeCard(card)
  }
...
function publicCard(card) {
  ...
  for (const key of [
    ...
    'estimated', 'bindProviders', 'windowDays', 'startedAt', 'resetAt', 'remainingDays',
  ]) {
