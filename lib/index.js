...
async function callHttp(config, source) {
  ...
  if (!response.ok) {
    ...
    throw new SourceError(String(response.status), `HTTP ${response.status}${detail === '' ? '' : ` · ${detail}`}`, source.id, redactSecrets(text.slice(0, 600), [headers.authorization, headers.cookie, init.body]))
  }
...
async function callConsoleGateway(config, source, cookie, apiName, secToken) {
  ...
  const text = await response.text()
  if (!response.ok) throw new SourceError(String(response.status), `HTTP ${response.status}`, source.id, redactSecrets(text.slice(0, 600), [cookie, secToken]))
  let payload
  try {
    payload = JSON.parse(text)
  } catch {
    throw new SourceError('BadResponse', '控制台返回了非 JSON（登录态多半失效，需重新复制 Cookie）', source.id, redactSecrets(text.slice(0, 300), [cookie, secToken]))
  }
  if (/NotAuthorised|NotLogined/i.test(JSON.stringify(payload).slice(0, 500))) {
    ...
    throw new SourceError(gatewayError, pickString(payload, ['data.errorMsg', 'errorMsg']) ?? '控制台网关未接受这份登录会话 Cookie', source.id, configlessTrim(payload, 0, [cookie, secToken]))
  }
  const code = pickString(payload, ['code']) ?? ''
  if (code !== '' && code !== '200' && payload?.successResponse !== true) {
    throw new SourceError(code, pickString(payload, ['message', 'msg']) ?? '网关未返回成功状态', source.id, configlessTrim(payload, 0, [cookie, secToken]))
  }
  const inner = pickPath(payload, 'data.DataV2.data')
  if (isPlainObject(inner) && inner.success === false) {
    throw new SourceError('ConsoleApiError', pickString(inner, ['message', 'errorMsg']) ?? '网关业务失败', source.id, configlessTrim(payload, 0, [cookie, secToken]))
  }
  const data = isPlainObject(inner?.data) ? inner.data : inner
  if (!isPlainObject(data)) throw new SourceError('BadResponse', '响应里找不到 DataV2 数据体（接口可能改版）', source.id, configlessTrim(payload, 0, [cookie, secToken]))
  return data
}
...
async function postForm(url, body, timeoutMs) {
  ...
  if (response.status === 401 || response.status === 403) {
    throw new SourceError(String(response.status), '鉴权被拒（检查 Key 权限与所属账号）', url, redactSecrets(text.slice(0, 600), [body]))
  }
  return text
}
