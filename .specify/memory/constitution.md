...
- 修订内容: 把"没有官方分母就永不出现百分比"限定为"不得**自行派生**百分比"。触发事实：
  ...
  真值，可以显示；**派生**出来的百分比仍无条件禁止。同时补两条硬约束：缺分母时**不画余量条**
  （条在视觉上是"余量/额度"，没有绝对分母支撑会与档位上限值混淆）、**不报绝对剩余量**（算不出来
  的东西不显示），并且必须留 `extra.denominatorFailed` 一类的诊断，不得静默降级。
...
（`used/total`、`100 − used%`、任何换算式都算派生）。上游接口**直接返回**的比例（如千问
`usage.per1MonthPercentage`）属官方真值，可以显示；但缺分母时**不画余量条、不报绝对剩余量**，
且必须留 `extra.denominatorFailed` 一类的诊断，不得静默降级成"无数据"。实测卡永不画余量条
（它压根没有官方读数）。一个计量窗口当且仅当该套餐确实返回了读数时才存在——档位配置里躺着的
上限值不是额度，只能进 `extra.*ConfiguredNoReading` 供 debug。
