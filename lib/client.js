...
/* 徽标绝不能把宿主的输入行挤到换行。两件事，缺一不可：
   1. 整条链要允许收缩（flex item 的 min-width 默认 auto，会顶在内容的 min-content 上）；
   2. 但只收缩不够——宿主那一行是 flex-wrap: wrap，分行按各项的 base size 决定，
      收缩只发生在分行之后。所以必须给一个硬上限。实测（1150/980/860 窗口都一样，
      输入列是固定宽的居中列）：徽标 ≤317px 一行，320px 起把模型选择器挤到第二行。
   上限取 312，留 5px 余量。被裁掉的只能是供应商名（完整名在 title 里），
   数字与「实测」pill 是产品立场，flex:none 保住。
...
.tpq-chip{display:inline-flex;align-items:center;gap:7px;height:24px;padding:0 8px;border-radius:7px;
 ...
 cursor:pointer;white-space:nowrap;min-width:0;max-width:min(312px,100%);overflow:hidden;
...
.tpq-close{flex:none;width:20px;height:20px;margin:-2px -4px 0 0;padding:0;display:inline-flex;
