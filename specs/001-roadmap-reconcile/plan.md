...
- **过度声称**：把①写成"已完成"不准确（A 档仍有 T1.4b / T1.5 / T1.8 未完成；B 档不做；C 档走实测窗口）
  → 必须写"部分完成"并指向不做记录（FR-004）。
- **不可核验断言**：每条状态都必须带可 grep 的位置（FR-002 / SC-001）。
- **与 README 冲突**：实现时对照 README 的 `## 明确不做的三类` 小节逐条对齐。
...
- **计划里缺的**：
  1. 没预见到**文档里写行号会腐烂**：我在修 B/C 措辞时把 §1 撑大两行，§2/§3 行号立刻再次错位。
     → 引用位置改用**标题锚点**。
     **反讽且必须记住**：Lens 1 复核我的修复提交时指出——我在写下这条教训的同一段里写了行号，
     而且写错了（写的 L57/L181/L276，实际 L57/L184/L279）。所以本条**不再给任何位置行号**。
...
- **落点**（本轮已全部应用）：
  ...
  - `workflow/REVIEW-CHECKLIST.md` 闭环段：+1 条——修完**通读被改段落**（不只 diff 行）：原地替换一段
    文字极易留下重复残段（本轮实测：三份评审都没抓到，我在复核时才自己发现 §1 状态行被留成了两段）。
  - 本条自身：**不再给任何位置行号**（Lens 1 指出我在写这条教训时写了行号且写错了——正是本条要防的事）。
  - Lens 1 的另外两条（README 章节行号错、①阶段漏掉 A 档自身推迟项）也已修，属"引用要精确"与
    "状态要完整"的同族问题，无需新增规则。