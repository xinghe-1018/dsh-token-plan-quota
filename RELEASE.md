...
## 0. 发布前唯一的闸门：`ROADMAP.md` 要不要公开

`ROADMAP.md` 里有本机的真实数字（DeepSeek 余额、Token Plan 剩余与档位 `specCode`、在用路由名）。
推送即公开（GitHub 会被索引，删掉提交也不保证缓存消失）。三选一：

```bash
# a) 脱敏后公开（推荐：把数字换成占位，保留决策理由）
#    我可以做，改完跑 npm run check 确认没牵连别的声明
# b) 只留本地：从跟踪里摘掉，历史里那份仍在（已提交的 14 个 commit 里有它）
git rm --cached ROADMAP.md && echo "ROADMAP.md" >> .gitignore
# c) 就这样公开
```

> 注意 (b) 的边界：`ROADMAP.md` 已经在**本地未推送**的提交里（`99f0a45` 起）。
> 没 push 之前 `git rm --cached` 就等于从未公开过；push 之后再撤就是"发布后删除"，性质不同。
